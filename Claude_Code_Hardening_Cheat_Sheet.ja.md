...
- **秘密の置き場所が増える** — Claude Code を使うと、API キーやトークンが平文で残りうる場所が増えます。設定ファイル、環境変数、そして会話の transcript です。クラウド同期の範囲に入っていれば、書いた瞬間に、同期しているすべての端末へ複製されます。ほかのリスクは設定を直せば止まりますが、**漏れた秘密だけは直せません**。ローテーションするしかありません。それで、この一点は他より慎重に扱います（§6）。（[OWASP LLM02: Sensitive Information Disclosure](https://genai.owasp.org/llm-top-10/)）
...
例外はありません。「あとで消すから」「ローカルだけだから」で書いた行が、そのままバックアップ・スクリーンショット・画面共有に乗ります。同期の対象になっているファイルなら、同期にも乗ります。クラウド同期をやめてください、という話ではありません。どこまで運ばれるかを数え上げるより、値を書かないほうが確実だ、ということです。gitignore されていても、同期からは守られません。
