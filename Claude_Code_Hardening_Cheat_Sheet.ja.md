...
- **秘密の置き場所が増える** — Claude Code を使うと、API キーやトークンが平文で残りうる場所が増えます。設定ファイル、環境変数、そして会話の transcript です。そのファイルがクラウド同期の範囲に入っていれば、書いた瞬間に全端末へ複製されます。ホームをまるごと同期している必要はなく、`~/Documents` の下にプロジェクトを置いているだけでも起こります。ほかのリスクは設定を直せば止まりますが、**漏れた秘密だけは直せません**。ローテーションするしかありません。それで、この一点は他より慎重に扱います（§6）。（[OWASP LLM02: Sensitive Information Disclosure](https://genai.owasp.org/llm-top-10/)）
...
例外はありません。「あとで消すから」「ローカルだけだから」で書いた行が、そのまま同期・バックアップ・スクリーンショット・画面共有に乗ります。そのファイルがクラウド同期の範囲に入っていれば、書いた瞬間に複製されます。`~/.claude/settings.json` ならホームや `~/.claude` を同期しているとき、プロジェクト側の `.claude/settings.local.json` なら、そのプロジェクトが `~/Documents` のような同期対象のフォルダにあるときです。ホームをまるごと同期していなくても起こります。見るべきはパスではなく実体の置き場所で、ファイルの本体を同期フォルダに置いてシンボリックリンクから参照している場合も、複製されるのは本体のほうです。gitignore されていても、同期からは守られません。
