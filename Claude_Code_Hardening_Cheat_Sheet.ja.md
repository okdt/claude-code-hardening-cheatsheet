...
Claude Code はユーザに代わってシェルコマンドの実行、ファイルの読み書き、外部サービスとの連携などを実施します。できることが多い分、意図しない操作もできてしまいます。この文書はそのリスクをコントロールするための文書です。初心者は、基本的に「サンドボックス」のところだけでもやっておいてください。技術者はチームの基本ポリシーを策定するのに参考にしてください。
...
Deny ルールで基本的なケースを止め、サンドボックスが境界の原則に基づいてアクセスを制限し、Hooks でさらに詳細な条件を設置できます。この3層の組み合わせで保護するイメージです。
...
| `git clean -f` | 追跡されていないファイルの完全削除。 |
| `git add . / -A` | すべてをステージング。`.env`、認証情報、巨大なバイナリを誤って含みかねない。 |
...
|--------|---------------------|------------|
| `open` | 「ファイルやURLを開くだけ」 | 任意のアプリ起動、フィッシングURL、ダウンロードファイルの実行。ブラウザ自動化ツール（Puppeteer等）は `open` コマンドを経由せず直接ブラウザを制御するため、影響なし。 |
...
| `gcloud --quiet` | GCP CLI を確認プロンプトなしで実行。 |
