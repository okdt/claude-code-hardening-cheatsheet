...
例外はありません。「あとで消すから」「ローカルだけだから」で書いた行が、そのまま同期・バックアップ・スクリーンショット・画面共有に乗ります。クラウド同期をやめてください、という話ではありません。どこまで運ばれるかを数え上げるより、値を書かないほうが確実だ、ということです。gitignore されていても、同期からは守られません。
