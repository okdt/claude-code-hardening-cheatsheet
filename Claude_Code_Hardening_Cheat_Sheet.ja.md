...
例外はありません。「あとで消すから」「ローカルだけだから」で書いた行が、そのまま同期・バックアップ・スクリーンショット・画面共有に乗ります。そのファイルがクラウド同期の範囲に入っていれば、書いた瞬間に複製されます。`~/.claude/settings.json` ならホームや `~/.claude` を同期しているとき、プロジェクト側の `.claude/settings.local.json` なら、そのプロジェクトが `~/Documents` のような同期対象のフォルダにあるときです。ホームをまるごと同期していなくても起こります。gitignore されていても、同期からは守られません。
