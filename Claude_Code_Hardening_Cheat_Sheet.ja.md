...
- **秘密の置き場所が増える** — Claude Code を使うと、API キーやトークンが平文で残りうる場所が増えます。設定ファイル、環境変数、そして会話の transcript です。クラウド同期の範囲に入っていれば、書いた瞬間に全端末へ複製されます。ほかのリスクは設定を直せば止まりますが、**漏れた秘密だけは直せません**。ローテーションするしかありません。それで、この一点は他より慎重に扱います（§6）。（[OWASP LLM02: Sensitive Information Disclosure](https://genai.owasp.org/llm-top-10/)）
