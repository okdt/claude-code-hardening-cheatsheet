...
- **More places for secrets to sit** — Using Claude Code introduces places where an API key or token can be exposed in plaintext: configuration files, environment variables, and the conversation transcript. If any of that lands inside something you sync to the cloud, it's copied to every machine you own the moment you write it. Every other risk here stops once you fix the setting. **A leaked secret doesn't.** Rotation is the only way out, which is why this one gets handled more carefully than the rest (Section 6). ([OWASP LLM02: Sensitive Information Disclosure](https://genai.owasp.org/llm-top-10/))
